...
const resolvers = {
  Query: { 
    ventas: () => q('SELECT id, fecha, total FROM ventas ORDER BY id'), 
    venta: (_p, { id }) => 
      q('SELECT id, fecha, total FROM ventas WHERE id = ?', [id]) 
      .then(f => f[0] || null), 
  }, 
  
  Venta: { 
    // 👇 ESTA ES LA LÍNEA QUE ACABAS DE REEMPLAZAR 👇
    detalle: (venta, _a, { cargadores }) => cargadores.detallePorVenta.load(venta.id),
    
    // Lo demás se mantiene igual
    clienteId: (venta) => venta.cliente_id, 
    cliente: async (venta) => { 
      const base = process.env.URL_USUARIOS || 'http://localhost:3000'; 
      const r = await fetch(base + '/v1/usuarios/' + venta.cliente_id); // (o la ruta que hayas dejado)
  ...
  Mutation: { 
    crearVenta: async (_p, { input }) => { 
      ...
      return { id: r.insertId, fecha: input.fecha, total }; 
    }, 
    // CORRECCIÓN: cambiarCantidad debe ir DENTRO de Mutation
